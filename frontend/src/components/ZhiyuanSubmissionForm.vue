<template>
  <div>
    ...
    <p
      ...
      class="success"
    >
      你已经在本网站完成了志愿填写，下边是你的填报情况。
    </p>
    <p
      v-else
      class="info"
    >
      下面，请你根据你所拥有的最新信息和推荐的名单选择第一批次(非定向)的志愿院校。
    </p>
    ...
    <div v-show="showAdmissionScores">
      <span>
        ...
        <el-button
          type="text"
          @click="admissionScoresDialog = true"
        >点击这里</el-button>查看往年相关一本院校录取分数线的数据
      </span>
      <el-dialog
        title="相关一本院校录取分数线"
        :visible.sync="admissionScoresDialog"
        width="800px"
        center
      >
        ...
        >
          <el-button
            type="primary"
            @click="admissionScoresDialog = false"
          >确定</el-button>
        </span>
      </el-dialog>
    </div>
    <div class="em-text">
      请一定注意：该网站主要目的在于帮助您了解志愿填报的更多信息，并不是真正的志愿填报。请务必通过宁夏教育考试院公布的高考志愿填报入口来填报志愿。
    </div>
    <el-divider />
    <el-row :gutter="20">
      <el-col
...
export default {
  ...
  computed: {
    ...
    zhiyuanColleges() {
      return this.hasCompletedZhiyuan ? this.$store.state.zhiyuanColleges : this.selectedColleges.map((college, i) => {
