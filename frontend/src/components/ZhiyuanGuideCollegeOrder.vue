<template>
  <div>
    ...
    <p>
      ...
      <span class="danger">按照错误的顺序排列，可能会导致您不能被理想的院校录取!</span>
    </p>
    <p>
      报考一批次志愿的时候，需要牢记以下
    ...
    <section ref="point1">
      <p>
        1. 平行志愿录取规则是“分数优先，遵循志愿”。在您的分数较高，多所学校可以录取您的情况下，
        <span class="danger">您一定会被自己志愿表中排位最前的志愿录取！</span>根据之前的填写，您的一批次志愿是：
      </p>
      <ReadOnlyZhiyuanForm />
      <p
      ...
      <p>因此，在考虑院校排列的时候，您需要考虑以下问题：如果在两所学校甲和乙都想录取您的时候，您会选择去甲上学，那么请一定将甲排在更靠前的位置上(比如，如果您在考虑 A 院校和 B 院校位置的填报，那么甲应该放在 A 院校的位置上，乙应该放在 B 院校的位置上)</p>
      <el-divider />
      <strong>现在，请回答下列问题，来确认您对上述内容的理解</strong>
      <div
    ...
    <section v-if="point1Answered" ref="point2">
      ...
      <p>在本科一批次可以填报 4 个高校平行志愿的情况下，如果您想争取上一所好院校的话:</p>
...
export default {
  ...
  computed: {
    ...
    point1Scenarios() {
      return [
        {
          description: `如果您的分数比${this.getCollegeName(
            1
          )}的分数线高，那么不论您在一批次 B 志愿，一批次 C 志愿，一批次 D 志愿的位置填写了什么学校，不论这些学校的分数线有多高/低，您都会被 ${this.getCollegeName(
            1
          )} 录取。`
        },
        {
          ...
          ${this.getCollegeName(
            1
          )} 的分数线低，那么不论您在一批次 C 志愿，一批次 D 志愿的位置填写了什么学校，不论这些学校的分数线有多高/低，您都会被 ${this.getCollegeName(
            2
          )} 录取`
        }
      ];
    ...
    point1Questions() {
      const questions = [
        {
          ...
          correctPrompt: "回答正确！",
          wrongPrompt:
            "回答错误，正确答案是第一项。因为您更愿意去甲学校，所以甲应该放在位置靠前的 C 志愿院校上。",
          description:
            "现在您准备把两所学校，甲和乙，放在一批次志愿表中 C 院校，D 院校的位置 上。如果当两所学校都想录取您时，您更愿意去甲，那么下列做法中正确的一项是",
          answerOptions: [
            {
              key: "A",
      ...
      if (this.intendedColleges.length > 3) {
        questions.push({
          answer: "A",
          correctPrompt: "回答正确！",
          wrongPrompt: `回答错误，正确答案是第一项。根据平行志愿的录取规则，您将不能被 A 志愿院校和 B 志愿院校录取，但可以被 C 志愿院校或 D 志愿院校录取，因为 C 志 愿院校比 D 志愿院校排位靠前，所以您一定会被 ${this.getCollegeName(
            3
          )} 录取。`,
          description: `假设您在分数线公布后发现自己的分数比 ${this.getCollegeName(
            3
      ...
      return [
        {
          ...
          correctPrompt: "回答正确！",
          wrongPrompt:
            "回答错误，正确答案是第三项。第一项中的策略会导致您失去冲刺好学校的机会。第二项中的策略可能会导致落榜的发生。第四项中的策略会导致 B,C 两个志愿院校被浪费，因为在平行志愿的录取规则中，您会直接被 A 志愿院校录取，B,C 两个志愿院校在 A 志愿院校录取您的情况下会被系统忽略，从而失去冲刺好学校的机会。只有第三选项的策略做到了冲，保结合，即没有放弃冲刺好学校的机会，也做到了风险最小化。",
          description:
            "如果您认为上一个好的院校比较重要，那么下列策略中较为合理的一项是 ___",
          answerOptions: [
            {
              key: "A",
